...
import { CChat } from '../controllers/chatList/ССhat';
    ...
    constructor(){
        ...
        if(this.unForms!==null){
            this.unForms.forEach((form:HTMLFormElement)=>{
                let inputs:NodeListOf<HTMLInputElement>|null =form.querySelectorAll('.un__form > input')
            ...
            this.inputs.forEach((input:HTMLInputElement)=>{
                ...
                if(result==null){
                    ...
                    if(this.inputs?.length==Object.keys(this.userData).length){
                        ...
                        if(location=="/"){
                            this.HTTPTransport.post("auth/signin",{method:"POST", data:this.userData}).then((ans)=>{
                               if(ans==="OK"){
                                    new Navigator(idea, "/chats")
                               }
                            })
                            ...
                            this.HTTPTransport.put("user/password",{method:"PUT",data:this.userData}).then(()=>{
                            })
                        }
                        this.userData={}
                    }
